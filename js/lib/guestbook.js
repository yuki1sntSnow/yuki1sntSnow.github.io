...
mixins.guestbook = {
    data() {
        return {
            form: {
                username: localStorage.getItem('guestbook_username') || '',
                email: localStorage.getItem('guestbook_email') || '',
                content: ''
            },
            messages: [],
            pagination: {
    ...
    methods: {
        ...
        async submitMessage() {
            ...
            try {
                ...
                if (result.success) {
                    // 保存用户名和邮箱到本地存储
                    localStorage.setItem('guestbook_username', this.form.username);
                    localStorage.setItem('guestbook_email', this.form.email);
